 ...
 *
 * In scenarios involving a custom audio source, you can use the `IAudioPcmDataSender` class to send PCM audio data directly to the audio track. If the audio track is disabled, the sent audio data is automatically discarded.
 */
class IAudioPcmDataSender : public RefCountInterface {
 public:
   ...
   * @param capture_timestamp The timestamp (ms) when the audio is captured.
  ...
  virtual int sendAudioPcmData(
      ...
      uint32_t capture_timestamp, // in ms
 ...
 *
 * In scenarios involving a custom audio source, you can use the `IAudioEncodedFrameSender` class to
...
class IAudioEncodedFrameSender : public RefCountInterface {
 public:
  /**
   * Sends the encoded audio frames to the local audio track.
   *
   * @param payload_data The pointer to the encoded audio frames.
   * @param payload_size The size of the encoded audio frames.
   * @param audioFrameInfo Information of the audio frame. See
...
class IMediaPacketReceiver {
 public:
   ...
   * @param options The packet information. See {@link media::base::PacketOptions PacketOptions}.
   *
   */
...
class IMediaControlPacketReceiver {
 public:
  /**
   * Occurs when the track receives a media control packet.
   *
   * @param packet The pointer to the media packet.
   * @param length The length of the packet.
   *
...
class IMediaControlPacketSender {
 public:
   ...
   *
   * @param userId The ID of the user to send the packet to.
   * @param packet The pointer to the packet.
   * @param length The packet size.
   *
   * @return
...
class IAudioSinkBase : public RefCountInterface {
 public:
   ...
   *
   * @param audioFrame The reference to the audio frame. See \ref media::base::AudioPcmFrame "AudioPcmFrame".
   * @return
   * - `true`: Success.
   * - `false`: Failure.
   */
...
class IAudioFilterBase : public RefCountInterface {
 public:
   ...
   * @param inAudioFrame The reference to the audio frame to adapt. See \ref media::base::AudioPcmFrame "AudioPcmFrame".
   * @param adaptedFrame The reference to the adapted audio frame. See \ref media::base::AudioPcmFrame "AudioPcmFrame".
   * @return
   * - `true`: Success.
...
class IAudioFilter : public IAudioFilterBase {
 public:
   ...
   * @return
   * - The actual size of the private property: Success.
   * - -1: Failure.
   */
   ...
   *
   * @note
   * Currently, you can only get the name by hardcoding. For example:
...
class IVideoFilterBase : public RefCountInterface {
 public:
   ...
   * @return
   * - `true`: Success.
   * - `false`: Failure. For example, the `IVideofilter` object drops the video frame.
   */
...
class IVideoFilter : public IVideoFilterBase {
 public:
  ...
  /**
   * This function is invoked right before data stream starts.
   * Custom filters can override this function for initialization.
   * @return
  ...
  virtual bool onDataStreamWillStart() { return true; }
   /**
   * This function is invoked right before data stream stops.
   * Custom filters can override this function for deinitialization.
   */
  virtual void onDataStreamWillStop() { }
  /**
