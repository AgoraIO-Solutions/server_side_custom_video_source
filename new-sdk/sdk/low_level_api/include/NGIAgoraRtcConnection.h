...
struct TConnectionInfo {
  ...
  TConnectionInfo() : id(-1), channelId(nullptr), state(CONNECTION_STATE_DISCONNECTED), localUserId(nullptr), internalUid(0) {}
};
...
/**
 * The method that the SDK processes the packet after receiving the packet.
 */
enum RECV_TYPE : uint8_t {
  /**
   * 0: The SDK processes the packet but does not deliver the unprocessed packet to the receiver.
   */
  RECV_MEDIA_ONLY = 0,
  /**
   * 1: The SDK does not process the packet but delivers the unprocessed packet to the receiver.
   */
  RECV_PACKET_ONLY,
  /**
   * 2: The SDK processes the packet and delivers the unprocessed packet to the receiver.
   */
...
struct RtcConnectionConfiguration {
  ...
  /**
   * The method that the SDK uses to process the packet after receiving the packet. See #RECV_TYPE.
   */
  RECV_TYPE recvType;
...
class INetworkObserver {
 ...
 public:
 /**
  * Occurs when bandwidth estimation information is updated.
  *
  * You can use this callback to adjust the video sending bitrate based
  * on the target video encoder bitrate.
  *
  * @param info The network information. See \ref agora::rtc::NetworkInfo "NetworkInfo".
  */
